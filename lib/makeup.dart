...
class _MakeupPageState extends State<MakeupPage> {
  ...
  List<dynamic> _filteredMakeupArtists = [];
  bool _loading = true;
  ...
  Future<void> fetchMakeupArtists() async {
    ...
    try {
      final response = await http.get(Uri.parse("http://10.13.29.36:5000/api/makeup/all"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          _makeupArtists = data;
          _filteredMakeupArtists = data;
          _loading = false;
        });
      } else {
  ...
  void applyFilters() {
    setState(() {
      _filteredMakeupArtists = _makeupArtists.where((m) {
  ...
  void clearAllFilters() {
    setState(() {
      ...
      _filteredMakeupArtists = List.from(_makeupArtists);
    });
  }
            ...
            children: [
              ...
              Expanded(
                child: _loading
                    ...
                    : _filteredMakeupArtists.isEmpty
                        ? const Center(child: Text("No makeup artists found"))
                        : ListView.builder(
                            itemCount: _filteredMakeupArtists.length,
                            itemBuilder: (context, index) {
                              final makeup = _filteredMakeupArtists[index];
                              final imageUrl = (makeup["images"] != null && makeup["images"].isNotEmpty)
                                  ? makeup["images"][0]
                                  : "https://via.placeholder.com/150";

                              final categories = (makeup["categories"] ?? {}) as Map;
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Text(
                                            makeup["makeupName"] ?? "Unnamed Makeup Artist",
                                            style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                                          ),
                                          const SizedBox(height: 4),
                                          Text(
                                            makeup["shortDescription"] ?? "",
                                          ...
                                          Container(
                                            ...
                                            child: Text("${makeup["experience"] ?? 0} years experience"),
                                          ),
