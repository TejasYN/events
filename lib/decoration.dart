...
class _DecorationPageState extends State<DecorationPage> {
  ...
  List<dynamic> _filteredDecorations = [];
  bool _loading = true;
  ...
  Future<void> fetchDecorations() async {
    ...
    try {
      final response =
          await http.get(Uri.parse("http://10.13.29.36:5000/api/decoration/all"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          _decorations = data;
          _filteredDecorations = data;
          _loading = false;
        });
      } else {
  ...
  void applyFilters() {
    setState(() {
      _filteredDecorations = _decorations.where((d) {
        final cityMatch =
            selectedCities.isEmpty || selectedCities.contains(d["city"]);
        final categories = (d["categories"] as Map?)?.keys.toList() ?? [];
        ...
        final experienceMatch =
            selectedExperience.isEmpty || _matchExperience(d, selectedExperience);
        return cityMatch && categoryMatch && experienceMatch;
      }).toList();
    });
  }

  bool _matchExperience(dynamic d, Set<String> selections) {
    final experience = (d["experience"] ?? 0) as num;
          ...
          if (filterType == "City") {
            ...
            final options = experienceOptions.map((opt) {
              ...
              return CheckboxListTile(
                ...
                onChanged: (val) => sheetSetState(() {
                  if (val == true) {
                    selectedExperience.add(opt["id"]);
                  } else {
                    selectedExperience.remove(opt["id"]);
                  }
                }),
              );
            }).toList();
            ...
            children: [
              ...
              Expanded(
                child: _loading
                    ...
                    : _filteredDecorations.isEmpty
                        ? const Center(child: Text("No decorations found"))
                        : ListView.builder(
                            itemCount: _filteredDecorations.length,
                            itemBuilder: (context, index) {
                              final deco = _filteredDecorations[index];
                              final imageUrl =
                                  (deco["images"] != null && deco["images"].isNotEmpty)
                                      ? deco["images"][0]
                                      : "https://via.placeholder.com/150";
                              final categories =
                                  (deco["categories"] ?? {}) as Map;
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            child: Text(
                                                "${deco["experience"] ?? 0} years experience"),
                                          ),
  ...
  Widget filterButton(String label) {
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor:
            active ? Colors.redAccent : Colors.white.withOpacity(0.9),
