...
class PhotographyDashboard extends StatefulWidget {
  const PhotographyDashboard({super.key});
...
class _PhotographyDashboardState extends State<PhotographyDashboard> {
  ...
  setState(() {
    ...
    businessNameController.text = vendor["businessName"] ?? "My Studio";
    gstin = vendor["gstin"] ?? "";
    email = vendor["email"] ?? "";
  ...
  Future<void> _updatePhotographyDetails() async {
    ...
    final body = {
      "vendorId": vendorId,
      "studioName": businessNameController.text,
    ...
    final response = await http.put(
      Uri.parse("$baseUrl/api/photography/update"),
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      try {
        ...
        final photography = data["photography"] ?? {};
        final newVendorId = photography["vendorId"] ?? photography["vendorld"] ?? vendorId;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Photography details updated successfully")),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    var request = http.MultipartRequest(
      "POST",
      Uri.parse("$baseUrl/api/upload/photography"),
    );
  ...
  void _deleteImage(int index) async {
    ...
    final uri = Uri.parse(
        "$baseUrl/api/upload/photography/$vId?imageUrl=${Uri.encodeComponent(imageUrl)}");
  ...
  Widget buildImageUploadSection() {
    return Column(
      ...
      children: [
        const Text("Upload Photography Works",
  ...
  Widget buildCategorySection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Photography Categories",
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        ...categories.map((c) {
          return Row(
  ...
  Widget buildPhotographerDetails() {
    return Column(
      children: [
        TextField(
        ...
        ElevatedButton(
          onPressed: _updatePhotographyDetails,
