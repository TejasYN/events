...
class _MusicDashboardState extends State<MusicDashboard> {
  ...
  Future<void> _loadVendorData() async {
    ...
    setState(() {
      ...
      businessNameController.text = vendor["businessName"] ?? "My Music Group";
      gstin = vendor["gstin"] ?? "";
      email = vendor["email"] ?? "";
  ...
  Future<void> _updateMusicDetails() async {
    ...
    final body = {
      "vendorId": vendorId,
      "musicName": businessNameController.text,
    ...
    final response = await http.put(
      Uri.parse("$baseUrl/api/music/update"),
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      try {
        ...
        final newVendorId =
            music["vendorId"] ?? music["vendorld"] ?? vendorId;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Music details updated successfully")),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    var request = http.MultipartRequest(
      "POST",
      Uri.parse("$baseUrl/api/upload/music"),
    );
  ...
  void _deleteImage(int index) async {
    ...
    final uri = Uri.parse(
        "$baseUrl/api/upload/music/$vId?imageUrl=${Uri.encodeComponent(imageUrl)}");
  ...
  Widget buildImageUploadSection() {
    return Column(
      ...
      children: [
        const Text("Upload Music Group Photos",
  ...
  Widget buildMusicDetails() {
    return Column(
      children: [
        TextField(
          controller: businessNameController,
          decoration: const InputDecoration(
              labelText: "Group / Band Name", prefixIcon: Icon(Icons.music_note)),
        ),
        TextField(
          ...
          children: [
            UserAccountsDrawerHeader(
              ...
              currentAccountPicture: const CircleAvatar(
                ...
                child: Icon(Icons.music_note, size: 40, color: Colors.red),
              ),
            ),
            ListTile(
